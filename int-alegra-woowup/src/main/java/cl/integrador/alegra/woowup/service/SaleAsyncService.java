...
public class SaleAsyncService {
    ...
    public void procesoAsyncDelaVenta(String jsonAlegra, Cliente clienteBD, String idCliente, String accessKey) {
        ...
        procesarEventoVenta(idDataLog, jsonAlegra, clienteBD, idCliente);
    ...
    private void procesarEventoVenta(Long idDataLog, String jsonAlegra, Cliente clienteBD, String idCliente) {
        JsonObject jsonObject = JsonParser.parseString(jsonAlegra).getAsJsonObject();
        String idFactura = jsonObject
                .getAsJsonObject("message")
                .getAsJsonObject("invoice")
                .get("id")
                .getAsString();
        jsonAlegra = alegra2Service.getInfoFactura(idFactura , clienteBD.getKeyAlegra(), 1);
        if (jsonAlegra != null) {
            jsonObject = JsonParser.parseString(jsonAlegra).getAsJsonObject();
            String idSucursal = jsonObject
                    .getAsJsonObject("costCenter")
                    .get("code")
                    .getAsString();
            ClienteSucursal sucursal = userSucursalDataRepository.findByClientAndSucursal(idCliente, idSucursal, true);
            if (sucursal == null) {
                manejarProcesoDeLaVenta(idDataLog, jsonAlegra, clienteBD);
            } else {
                log.warn("{} Se ignora la informacion por que la sucursal {} no esta autorizada", idDataLog, idSucursal);
                cerrarUnDataLog(idDataLog, "NOK", "Se ignora la informacion por que la sucursal no esta autorizada",
                        "La sucursal no permitida es : " + idSucursal);
            }
        } else {
            log.error("{} Problemas al obtener info de ALEGRA de : {}", idDataLog, idFactura);
            cerrarUnDataLog(idDataLog, "NOK", "Error en el proceso de leer desde Alegra los datos del cliente",
                    "");
        }
    }
    ...
    private void manejarProcesoDeLaVenta(Long idDataLog, String jsonAlegra, Cliente clienteBD) {
